...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    grouped_data = delivered.groupby("order_id")[
        ["freight_value", "product_weight_g"]
    ].sum()
    aggregations = grouped_data.reset_index()
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    filtered_dates = filtered_dates.copy()
    filtered_dates.loc[:, "date_column"] = filtered_dates[
        "order_purchase_timestamp"
    ].dt.date
    grouped_days = filtered_dates.groupby("date_column")["order_id"].count()
    order_purchase_ammount_per_date = grouped_days.reset_index()
    ...
    for i in list(order_purchase_ammount_per_date["date_column"]):
        if i in list(pd.to_datetime(holidays["date"]).dt.date):
            holidays_boolean.append(True)
        else:
            holidays_boolean.append(False)

    result_df = pd.DataFrame(
        {
            "order_count": list(order_purchase_ammount_per_date["order_id"]),
            "date": list(order_purchase_ammount_per_date["date_column"]),
            "holiday": holidays_boolean,
        }
    )
